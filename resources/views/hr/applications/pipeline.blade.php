...
<section class="max-w-7xl mx-auto px-6 py-8">
    <div class="mb-8">
        ...
        <p class="text-gray-500 mt-1">Manage candidates through all recruitment stages</p>
    </div>
    ...
    <div class="bg-white rounded-2xl p-6 shadow-sm border mb-8">
        <div class="flex flex-wrap items-center justify-center gap-2 text-xs font-semibold">
            ...
            <span class="px-3 py-2 bg-green-100 text-green-700 rounded-lg">✅ Doc Verified</span>
            <i class="fas fa-arrow-right text-gray-400"></i>
    ...
    <div class="bg-white rounded-2xl shadow-sm border overflow-hidden">
        <div class="p-4 border-b bg-gray-50 flex gap-2 flex-wrap">
            ...
            <a href="?status=document_verified" class="px-4 py-2 rounded-lg text-sm font-semibold transition {{ request('status')=='document_verified'?'bg-green-100 text-green-700':'bg-white text-gray-600 hover:bg-gray-100' }}">✅ Doc Verified</a>
            ...
            <a href="?status=written_exam" class="px-4 py-2 rounded-lg text-sm font-semibold transition {{ request('status')=='written_exam'||!request('status')?'bg-purple-100 text-purple-700':'bg-white text-gray-600 hover:bg-gray-100' }}">✍️ Written Exam</a>
                            ...
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-4 py-4">
                                    <div class="flex items-center justify-end gap-2">
                                        <!-- DOCUMENT VERIFIED → Move to Shortlisted -->
                                        @if($app->status == 'document_verified')
                                            <form action="{{ route('hr.applications.update-status', $app) }}" method="POST">
                                                @csrf
                                                <input type="hidden" name="status" value="shortlisted">
                                                <button class="px-3 py-1.5 bg-yellow-100 text-yellow-700 rounded-lg text-xs font-bold hover:bg-yellow-200">
                                                    ⭐ Shortlist
                                                </button>
                                            </form>
                                        @endif
                                        
                                        <!-- SHORTLISTED → Written Exam -->
                                        @if($app->status == 'shortlisted')
                                            <form action="{{ route('hr.applications.update-status', $app) }}" method="POST">
                                                @csrf
                                                <input type="hidden" name="status" value="written_exam">
                                                <button class="px-3 py-1.5 bg-purple-100 text-purple-700 rounded-lg text-xs font-bold hover:bg-purple-200">
                                                    → Written Exam
                                                </button>
                                            </form>
                                        @endif
                                        ...
                                        @if($app->status == 'selected')
                                            <a href="{{ route('hr.offer-letters.create', $app) }}" class="px-3 py-1.5 bg-blue-100 text-blue-700 rounded-lg text-xs font-bold hover:bg-blue-200">
                                                📄 Offer Letter
                                            </a>
                                        @endif

                                        <!-- REJECT (available for all except selected/rejected) -->
                                        @if(!in_array($app->status, ['selected', 'rejected']))
                                            <form action="{{ route('hr.applications.update-status', $app) }}" method="POST"
                                                onsubmit="return confirm('Reject this application?')">
                                                @csrf
                                                <input type="hidden" name="status" value="rejected">
                                                <button class="px-3 py-1.5 bg-red-50 text-red-500 rounded-lg text-xs font-bold hover:bg-red-100">❌ Reject</button>
                                            </form>
                                        @endif

                                        <a href="{{ route('applicant.applications.show', $app) }}" class="p-1.5 text-gray-400 hover:text-[#0a7aa8] rounded-lg"><i class="fas fa-eye"></i></a>
                                    </div>
                                </td>
                            </tr>
                        @endforeach
