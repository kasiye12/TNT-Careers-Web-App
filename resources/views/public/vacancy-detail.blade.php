...
<section class="hero-gradient text-white relative overflow-hidden">
    ...
    <div class="max-w-5xl mx-auto px-6 py-10 lg:py-14 relative">
        <nav class="flex items-center gap-2 text-sm text-white/60 mb-4">
            ...
            <i class="fas fa-chevron-right text-[10px]"></i>
            <span class="text-white/80">{{ Str::limit($vacancy->title, 40) }}</span>
        </nav>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white rounded-2xl p-6 shadow-md border border-gray-100 sticky top-24">
                ...
                <dl class="space-y-4">
                    ...
                    <div class="flex items-center justify-between py-2 border-b border-gray-50">
                        ...
                        <dd class="text-sm font-semibold text-gray-900">{{ number_format($vacancy->views_count ?? 0) }}</dd>
                    </div>
                    <div class="flex items-center justify-between py-2 border-b border-gray-50">
                        <dt class="text-sm text-gray-500 flex items-center gap-2">
                            <i class="fas fa-hourglass-end text-gray-300 w-4"></i> Deadline
                        </dt>
                        <dd class="text-sm font-bold {{ $vacancy->closing_date->isPast() ? 'text-red-500' : 'text-gray-900' }}">
                    ...
                    <div class="flex items-center justify-between py-2">
                        ...
                        <dd class="text-sm font-semibold text-gray-900">
                            @if($vacancy->salary_type == 'negotiable') Negotiable (Attractive)
                            @elseif($vacancy->salary_type == 'scale') As per Company Scale
                            @else {{ number_format($vacancy->salary_amount) }} {{ $vacancy->salary_currency }}
                            @endif
                        </dd>
                    </div>
                </dl>
                ...
                <div class="mt-6 space-y-3">
                    ...
                    @else
                        ...
                        <p class="text-center text-xs text-gray-400">
                            Don't have an account? <a href="{{ route('register') }}" class="text-[#0a7aa8] font-semibold">Register</a>
                        </p>
                    @endauth
                </div>

                <!-- Deadline Alert - Only show if within 7 days and not past -->
                @php $daysLeft = now()->diffInDays($vacancy->closing_date, false); @endphp
                @if($daysLeft > 0 && $daysLeft <= 7)
                    <div class="mt-4 p-3 bg-red-50 rounded-xl flex items-start gap-2">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-red-500">{{ (int)$daysLeft }} day(s) left to apply</p>
                        </div>
                    </div>
                @endif
            </div>
            ...
            <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                <h3 class="font-bold text-[#0b3b5a] mb-3">About TNT Construction</h3>
                <p class="text-sm text-gray-500 leading-relaxed">
                    Grade One General Contractor with over 20 years of experience building Ethiopia's infrastructure.
                </p>
                <div class="flex flex-wrap gap-2 mt-3 text-xs">
                    <span class="text-green-600 font-semibold">✅ GC-1 Certified</span>
                    <span class="text-green-600 font-semibold">✅ 20+ Years</span>
                    <span class="text-green-600 font-semibold">✅ 2,500+ Employees</span>
                </div>
            </div>
